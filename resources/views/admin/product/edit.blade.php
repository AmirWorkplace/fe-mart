    ...
    <div class="row g-3">
        <div class="col-12">
            <form action="{{ Route('admin.product.update', $product->id) }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="card-header">
                        <div class="d-flex align-items-center justify-content-between">
                            ...
                            <div class="flex-shrink-0">
                                ...
                                <button id="product-save-btn" class="btn btn-sm btn-success">Update Product</button>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-sm-6">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="number" class="form-control" id="discount" name="discount"
                                                placeholder="Discount Percentage" value="{{ $product->price->discount }}">
                                        </div>
                                    </div>
                                    <div class="col-sm-6">
                                    ...
                                    <div class="col-sm-6">
                                        ...
                                        @if ($product->more_images)
                                            ...
                                            <div id="preview_image" class="pt-2" style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 4px;">
                                                @foreach ($more_images as $key => $image)
                                                    <div class="position-relative" id="remove-other-img--{{ $key }}">
                                                        <img class="thumb" src="{{ asset($image) }}" alt="thumb-more-img">

                                                        <span class="remove-other-img" remove-other-img-id="#remove-other-img--{{ $key }}" remove-other-img="{{ $image }}">x</span>
                                                    </div>
                                                @endforeach
                                            </div>
                                        @else
                            ...
                            <div class="col-lg-6">
                                <div class="row g-3">
                                    <div class="col-12">
                                        <div class="row g-3">
                                            <div class="col-sm-6">
                                                <label class="form-label" for="code"><b>Product ID</b> <b id="code_vmsg"></b></label>
                                                <input name="code" id="code" class="form-control custom-input" placeholder="#******" value="{{ $product->code }}" required />
                                            </div>
                                        
                                            <div class="col-sm-6">
                                                <label class="form-label" for="parent_category"><b>Parent Category</b></label>
                                                <select name="main_category_id[]" id="parent_category" class="select form-select" required data-placeholder="Select Category.." multiple>
                                                    @foreach ($get_parent_categories as $key => $category)
                                                        @if (in_array($category->id, $category_ids->main_category_id))
                                    ...
                                    <div class="col-sm-6">
                                        ...
                                        <select name="child_category_id[]" id="child_category" class="select form-select"
                                            data-placeholder="Choose.." multiple>
                                            @foreach ($child_categories as $key => $category)
                                                @if (in_array($category->id, $category_ids->child_category_id))
                                                    <option value="{{ $category->id }}" selected>
                                                        {{ $category->name }}</option>
                                                @else
                                                    <option value="{{ $category->id }}">
                                    ...
                                    <div class="col-sm-6">
                                        ...
                                        <select name="subchild_category_id[]" id="subchild_category" class="select form-select"
                                            data-placeholder="Choose.." multiple>
                                            @foreach ($subchild_categories as $key => $category)
                                                @if (in_array($category->id, $category_ids->subchild_category_id))
                                                    <option value="{{ $category->id }}" selected>
                                                        {{ $category->name }}</option>
                                                @else
                                                    <option value="{{ $category->id }}">
                ...
                <div class="card mt-3">
                    <div class="card-body">
                            ...
                            <div class="row g-3 form-group mb-3">
                                ...
                                <div class="col-md-9">
                                    <select name="stock_check" id="stock_check"
                                        class="form-control select" data-placeholder="Stock Check">
                                            <option value="{{ $product->stock_check }}" selected>
                                            {{ $product->stock_check == 0 ? 'No' : ($product->stock_check == 1 ? 'Yes' : "Pre Order") }}
                                            </option>
                        ...
                        <div id="show-hide-div" style="display: {{ count(json_decode($product->choice_options)) > 0 ? 'none' : 'block' }};">
                            <div class="form-group row g-3 mb-3">
                                <label class="col-md-3 col-from-label">{{ __('Quantity') }}
                                    <span class="text-danger">*</span></label>
                                <div class="col-md-9">
                                    <input type="number" min="0" value="{{ $product->stocks->first()->qty ?? 0 }}"
                ...
                <input type="hidden" name="remove_other_img" value="[]">
            </form>
        </div>
    </div>
@endsection
...
@push('js')
    <script type="text/javascript">
        $(document).ready(function() {
            // remove others images handler in product edit mode
            let imageUrlStack = [];
            $('.remove-other-img').click(function(event){
                const imageUrl = $(event.target).attr('remove-other-img');
                imageUrlStack.push(imageUrl);

                // remove image 
                const removeId = $(event.target).attr('remove-other-img-id');
                $(removeId).remove();

                // console.log(imageUrlStack);
                $('[name="remove_other_img"]').val(JSON.stringify(imageUrlStack));
            });

            // top upload button handler
            $('#product-save-btn').click(function(){
                $('#choice_form').submit();
            });

            // product code validation
            $("#code").on('change keyup click', function(){
                $.ajax({
                    url: "{{ route('admin.validate-product-code') }}",
                    method: 'POST',
                    data: { code: $("#code").val() },

                    success: function(data){
                        if(data.status){
                            $("#code_vmsg").html('(<span class="text-danger">Not Unique</span>)');
                        }else{
                            $("#code_vmsg").html('(<span class="text-success">Perfect</span>)');
                        }
                    },
