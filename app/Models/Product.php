...
class Product extends Model
{
    ...
    public function getProductsByParentCategoryId(array|string $parent_category_id, string|NULL $product_type = null)
    {
        ...
        if($product_type){
            return $this->whereJsonContains('category_id->main_category_id', [$parent_category_id])->with('price')->where('status', true)->where('product_type', $product_type)->latest('updated_at')->get();
        }
        
        return $this->whereJsonContains('category_id->main_category_id', [$parent_category_id])->with('price')->where('status', true)->latest('updated_at')->get();
    }

    public function getProductsByChildCategoryId($child_category_id, string|NULL $product_type = null)
    {
        if($product_type){
            return $this->whereJsonContains('category_id->child_category_id', [$child_category_id])->with('price')->where('status', true)->where('product_type', $product_type)->latest('updated_at')->get();
        }
        
        return $this->whereJsonContains('category_id->child_category_id', [$child_category_id])->with('price')->where('status', true)->latest('updated_at')->get();
    }

    public function getProductsByChildrenCategoryId($children_category_id, string|NULL $product_type = null)
    {
        if($product_type){
            return $this->whereJsonContains('category_id->subchild_category_id', [$children_category_id])->with('price')->where('status', true)->where('product_type', $product_type)->latest('updated_at')->get();
        }
        
        return $this->whereJsonContains('category_id->subchild_category_id', [$children_category_id])->with('price')->where('status', true)->latest('updated_at')->get();
    }
