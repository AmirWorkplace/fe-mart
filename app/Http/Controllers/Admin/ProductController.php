...
class ProductController extends Controller
{
        ...
        if (request()->ajax()) {
            ...
            return DataTables::eloquent($model)
                ...
                ->addColumn('category', function ($row) {
                    $category_id = json_decode($row->category_id);
                    $parent_category_id =  $category_id->main_category_id;
                    
                    $categories = Category::select('id', 'name', 'status')
                        ->whereIn('id', $parent_category_id)
                        ->where('status', true)
                        ->latest('updated_at')
                        ->get();
                        
                    return $category_name = implode(', ', $categories->pluck('name')->toArray()); 
                })
                ->addColumn('regular_price', function ($row) {
                    return "<p class='text-center'>{$row->price->regular_price} TK</p>";
                })
                ->addColumn('sale_price', function ($row) {
                    return "<p class='text-center'>{$row->price->sale_price} TK</p>";
                }) 
                ...
                ->rawColumns(['checkbox', 'image','name', 'code', 'category', 'regular_price', 'sale_price', 'status', 'actions'])
                ->make(true);
        }
    ...
    {
        if (request()->ajax()) {
            $child_categories = Category::whereIn('parent_id', request('id'))->where('status', 1)->get();
    ...
    public function store(Request $request)
    {
        $request->validate(
            ...
            [
                'video.max' => 'Can not Upload video largest than 15MB'
            ]
        );
        ...
        $category_id = json_encode([
            'main_category_id'=> is_null($request->main_category_id) ? [] : $request->main_category_id,
            'child_category_id'=> is_null($request->child_category_id) ? [] : $request->child_category_id,
            'subchild_category_id'=> is_null($request->subchild_category_id) ? [] : $request->subchild_category_id,
        ]);
    ...
    // check product id is unique or not
    public function validateProductCode(Request $request){
        $is_exists_product_code = Product::where('code', $request->code)->exists(); 

        return $is_exists_product_code ? ["status"=> true] : ["status"=> false];
    }
    ...
    {
        $get_parent_categories = AdditionalDataResource::getParentCategories();
        $get_child_categories = [];
        $get_subchild_categories = [];
        $product = Product::findOrFail($id);
        $category_ids = json_decode($product->category_id);

        $parent_categories = Category::latest('updated_at')->whereIn('id', $category_ids->main_category_id)->get();
        $child_categories = AdditionalDataResource::getChildCategoriesByParentId($category_ids->main_category_id);
        ...
        if (request()->ajax() && request()->has('getCategories')) {
            $get_child_categories = AdditionalDataResource::getChildCategoriesByParentId(request('id'));
            return response()->json(['status' => 'success', 'child_categories' => $get_child_categories]);
        }

        if (request()->ajax() && request()->has('getChildrenCategories')) {
            $get_subchild_categories = AdditionalDataResource::getChildrenCategoriesByParentId(request('id'));
            return response()->json(['status' => 'success', 'child_categories' => $get_subchild_categories]);
        }
        ...
        return view('admin.product.edit', compact('product', 'category_ids', 'get_parent_categories', 'get_child_categories', 'get_subchild_categories', 'parent_categories', 'child_categories', 'subchild_categories', 'brands'));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        // get out updater product
        $product = Product::findOrFail($id);

        // get remove images
        $remove_other_img = json_decode($request->input('remove_other_img')); 

        if(!empty($remove_other_img)) {
            $get_other_img = explode('|', $product->more_images);

            // find out the different imag
            $get_diff_images = array_diff($get_other_img, $remove_other_img);

            foreach($remove_other_img as $img) {
                if(file_exists($img)){
                    unlink($img);
                }
            }

            $product->more_images = implode("|", $get_diff_images);
            $product->save();
        }
        ...
        function createSlug($related_name, $ignore_id){
            $slug = Str::slug($related_name);
            $same_slug_count = Product::withTrashed()->where('slug', 'LIKE', $slug . '%')->whereNotIn('id', [$ignore_id])->count();
            $slug_suffix = $same_slug_count ? '-' . ($same_slug_count + 1) : '';
            $slug .= $slug_suffix;

            if(Product::where('slug', $slug)->exists()){
                return $slug .= $same_slug_count ? '-' . ($same_slug_count + 1) : '';
            }
        ...
        $product->update([
            ...
            'name' => $request->name,
            'slug' => createSlug($request->name, $id),
            'thumbnail' => $thumbnail_path_name,
            'more_images' => $more_images_path_names,
            'stock_check' => $request->stock_check,
            'short_description' => $request->short_description,
