...
class FrontpageController extends Controller
{
    ...
    public function products($slug = NULL)
    {
        if (!is_null($slug)) {
            ...
            $products = $products_instance->getProductsByCategoryId($category->id, 9, null);
        } else {
            $category = NULL;
    ...
    {
        $product = Product::with(['price', 'stocks'])->where('slug', $slug)->where('status', 1)->first();
        ...
        return view('frontend.single_product', compact('product', 'related_products', 'shareComponent', 'categories'));
    }
